...
export default function ViewAllTours({ user, tours }: Props){
        ...
        setTimeout(() => {
            ...
            if (searchQuery) {
                results = results.filter(tour =>
                    ...
                    tour.tour_description.toLowerCase().includes(searchQuery.toLowerCase()) ||
                    tour.location.name.toLowerCase().includes(searchQuery.toLowerCase())
                );
            }
            if (selectedCategory) {
                results = results.filter(tour => tour.categories[0].name === selectedCategory);
            }
            switch (sortOrder) {
                case 'price-low': results.sort((a, b) => a.tour_price - b.tour_price); break;
                case 'price-high': results.sort((a, b) => b.tour_price - a.tour_price); break;
                case 'rating': default: results.sort((a, b) => b.tour_rating - a.tour_rating); break;
            }
        ...
        <Box minH="100vh" bg={overallBg} animation={`${fadeIn} 0.5s ease-out`}>
            ...
            <Container maxW="container.xl" py={{ base: 6, md: 10 }}>
                ...
                <Breadcrumb spacing="8px" separator={<ChevronRightIcon color="gray.500" />} mb={6}>
                    <BreadcrumbItem>
                        <BreadcrumbLink onClick={() => navigateTo('/dashboard')} color={secondaryTextColor} _hover={{ color: primaryColor }}>Home</BreadcrumbLink>
                    </BreadcrumbItem>
                                ...
                                <Box p={5}>
                                    <HStack justify="space-between" mb={1.5}>
                                        <Text fontSize="sm" color={primaryColor} fontWeight="semibold" textTransform="uppercase" letterSpacing="wider">
                                            {tour.location?.name ?? 'Unknown Location'} • {categories.find(c => c.value === tour.categories[0]?.name)?.icon} {tour.categories[0]?.name ?? 'Uncategorized'}
                                        </Text>
                                    ...
                                    <Flex justify="space-between" align="center" mt={3}>
                                        <Box>
                                            <Text fontWeight="black" color={primaryColor} fontSize="lg">{formatPrice(tour.tour_price)}</Text>
