...
const menuItems: MenuItem[] = [
  { label: 'Explore', path: '/ViewAllTours', iconString: '🗺️' },
...
export default function Dashboard({ user, tours, featuredGuides, transactions}: Props) {


  // Don't Enable unless you are trying to debug data passing!
  // console.log("PROPS RECEIVED FROM LARAVEL:", {transactions, featuredGuides});
      ...
      <Container maxW="container.xl" py={{ base: 6, md: 10 }} position="relative" zIndex={1}>
        ...
        <Box mb={16}>
          <SectionHeader title="Featured Destinations" subtitle="Handpicked adventures just for you." icon={"🌟"}>
            ...
            <Link href='/ViewAllTour'>
              <Button {...secondaryButtonStyle} size="sm" h="42px" rightIcon={<ArrowForwardIcon ml={1} />}>View All</Button>
            </Link>
              ...
              <Link href={route('tour.show', { tour: tour.id, slug: tour.slug })}>
                ...
                >
                  <Box position="relative" h="250px" overflow="hidden">
                    <Image 
                    src={coverImage ? coverImage.image_path : "https://via.placeholder.com/150x140?text=Image"}
                    alt={coverImage ? coverImage.image_caption : tour.name}
                    w="full" h="full" 
                    objectFit="cover" 
                  ...
                  <Box p={{ base: 4, md: 5 }}>
                    ...
                    <Text color={secondaryTextColor} mb={3} fontSize="sm" lineHeight="1.6" noOfLines={2}>{tour.tour_description}</Text>
          ...
          <Box>
            ...
            {upcomingTours.length > 0 ? (
              <VStack spacing={5} align="stretch">
                {upcomingTours.map((upcomingTour, index) => (
                  <Box key={upcomingTour.id}
                  ...
                  >
                    <Flex direction={{ base: 'column', sm: 'row' }} p={4}>
                      <Image
                        src={upcomingTour.tour.images[0].image_path} alt={upcomingTour.tour.slug}
                        ...
                        onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                          console.error(`Failed to load image for ${upcomingTour.tour.name}:`, (e.target as HTMLImageElement).src);
                        }}
                      />
                      <Box flex="1" display="flex" flexDirection="column" justifyContent="space-between">
                        <Box>
                          <Flex justify="space-between" align="flex-start" mb={1}>
                            <Heading size="sm" color={primaryTextColor} fontWeight="semibold" noOfLines={2}>{upcomingTour.tour.name}</Heading>
                            <Badge colorScheme={upcomingTour.payment_status === 'paid' ? 'green' : upcomingTour.payment_status === 'unpaid' ? 'yellow' : 'red'} variant="subtle" fontSize="xs" px={2.5} py={1} borderRadius="md">{upcomingTour.payment_status}</Badge>
                          </Flex>
                          <HStack spacing={1.5} color={secondaryTextColor} fontSize="xs" mb={1}>
                            ...
                            <Text>{upcomingTour.tour.location.name}</Text>
                          </HStack>
                          <HStack spacing={1.5} color={secondaryTextColor} fontSize="xs" mb={3}>
                            ...
                            <Text>{upcomingTour.tour_date} at {upcomingTour.tour.tour_start_time}</Text>
                          </HStack>
                        </Box>
                        <Flex gap={3} mt="auto" alignSelf={{ base: 'stretch', sm: 'flex-end' }}>

                          <Link href='/Bookings'>
                            <Button {...primaryButtonStyle} size="xs" px={4} h="36px">View in Booking</Button>
                          </Link>
              ...
              <VStack bg={cardBg} p={{ base: 6, md: 10 }} borderRadius="xl" boxShadow="lg" textAlign="center" border="1px solid" borderColor={subtleBorderColor} spacing={4} minH="280px" justifyContent="center">
                ...
                <Link href='/tours'>
                  ...
                  <Button {...primaryButtonStyle} px={6} h="42px" mt={2} rightIcon={<Text as="span" ml={1}>🚀</Text>}>Let's Find One!</Button>
          ...
          <Box>
            ...
            <VStack spacing={4} align="stretch">
              {featuredGuides.map((guide, index) => (
                <Link href={route('guideprofile.userview', { guide: guide.id})}>
                  <Box
                    key={guide.id}
                  ...
                  >
                    <Flex align="center">
                      <Box position="relative" mr={3} flexShrink={0}>
                        <Avatar src={guide.profile_picture} name={guide.name} boxSize="60px" borderRadius="lg" boxShadow="md" />
